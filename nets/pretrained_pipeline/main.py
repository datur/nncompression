...
def cli(model, config):
    ...
    click.echo(f'chosen model is  {model}')
    click.echo(f'chosen config is {config["name"]}')
...
def main(model, config):
    ...
    inceptionv3 = inception.inception_v3(pretrained=True, device=device)

    print(summary(inceptionv3, (3, 32, 32)))
    ...
    torch_out = inceptionv3(x)

    torch.onnx.export(inceptionv3, x, 'inceptionv3.onnx', export_params=True,
